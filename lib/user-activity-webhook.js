        ...
        case 'POST':
            ...
            if (typeof payload.for_user_id === 'string') {
                ...
                for (let eventName in payload) {
                    ...
                    if (Array.isArray(events)) {
                        for (let i = 0; i < events.length; i++) {
                            ...
                            middleware.emit('event', eventName, payload.for_user_id, events[i], payload);
                        }
                    } else {
                        if (emitter) emitter.emit(eventName, events);
                        middleware.emit('event', eventName, payload.for_user_id, events, payload);
                    }
                }
                return;
                ...
                if (payload.user_event.revoke && typeof payload.user_event.revoke === 'object') {
                    ...
                    middleware.emit('event', 'revoke', payload.user_event.revoke.source.user_id, payload.user_event.revoke, payload);
                    return;
                }
            }
